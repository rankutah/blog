---
title: "Dentist in Pleasant Grove, UT | Rod Maxfield, DDS"
url: "/"
description: "Family dentist in Pleasant Grove, UT offering comprehensive preventive, restorative, and implant dentistry with compassionate care."
---

{{< section img="/media/hero.jpg" bleed="true" overlay="true" overlayShade="bg-black/40" imgStyle="height: 100vh" yBottom="md" align="center" spacer="false" >}}
# Dentistry Making a Difference {.text-white}
Compassionate care, modern technology, and personalized treatment options.
{.text-white}
{{<button url="tel:+18017852631" text="Call (801) 785-2631">}}{{<button url="/contact" text="Contact">}}

{{< /section >}}


{{< section >}}

{{< cols min="18rem" gap="2rem" v="start">}}

{{< col card="true" y="start" >}}

## Comprehensive Family Dentistry

From preventive care to restorative treatment and oral surgery, we provide thoughtful, personalized care for all ages.

{{< /col >}}

{{< col card="true" y="start" >}}

## Visit or message us

{{< button url="tel:+18017852631" text="Call (801) 785-2631" >}} {{< button url="/contact" text="Contact & Directions" >}}

{{< /col >}}

{{< /cols >}}

{{< /section >}}

{{< section >}}

## What Our Patients Say

{{< elfsight id="d8670cc6-9dfc-4864-96ea-929496ddd5fc" >}}

{{< /section >}}

{{< section >}}

## Featured Services

{{< cols min="18rem" gap="2rem" v="start" >}}

{{< col card="true" y="start" >}}
## Preventive

- [Hygiene Basics](/services/hygiene-basics/)
- [Fluoride](/services/fluoride/)
- [Infection Control](/services/infection/)
- [Gum Disease](/services/gum-disease/)
- [Mouth Guards](/services/mouth-guards/)

{{< /col >}}

{{< col card="true" y="start" >}}
## Aesthetics

- [Bridges](/services/bridges/)
- [Crowns](/services/crowns/)
- [Dentures](/services/dentures/)
- [Fillings](/services/fillings/)
- [Upgrades](/services/upgrades/)

{{< /col >}}

{{< col card="true" y="start" >}}
## Implants & Oral Surgery

- [Implants](/services/implants/)
- [Wisdom Teeth](/services/wisdom-teeth/)
- [Bone Grafting](/services/bone-grafting/)
- [PRF](/services/prf/)
- [Sedation](/services/sedation/)

{{< /col >}}

{{< /cols >}}

{{< button url="/services/" text="View All Services" >}}
{.center}

{{< /section >}}

{{< section >}}

## Why Patients Choose Us

### Quick Facts
- Decades of continuous family dental care in Pleasant Grove
- Comprehensive services (preventive, implants, oral surgery, aesthetics) in one location
- Same‑day help for urgent dental concerns when possible
- Thoughtful, low‑pressure treatment planning

### Credentials & Professional Involvement
- Creighton School of Dentistry training (with University of Utah School of Medicine collaboration)
- Active ADA member
- Former President – Utah Academy of General Dentistry
- Ongoing continuing education in implant systems, materials, imaging, and sedation

{{< /section >}}

{{< section >}}

<div class="text-center max-w-3xl mx-auto">
	<h3 class="text-2xl font-semibold mb-2">Ready for a healthier smile?</h3>
	<p class="mb-4">Call now or find our location. We’ll respond quickly during office hours.</p>
	<div class="flex flex-wrap items-center justify-center gap-3">
		{{< button url="tel:+18017852631" text="Call (801) 785-2631" >}}
		{{< button url="https://www.google.com/maps/search/?api=1&query=Rod+Maxfield+DDS+Pleasant+Grove+UT" text="Get Directions" >}}
	</div>
</div>

{{< /section >}}

