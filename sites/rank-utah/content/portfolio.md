---
title: "Portfolio"
description: "A portfolio of custom website design projects for local businesses"
layout: "flowbite"
url: "/portfolio"
---

# Website Portfolio
{.center}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://kandee4utah.com/" target="_blank" rel="noopener">
	{{< img src="/media/kandee-myers-politics-website-state-senate.avif" alt="Kandee 4 Utah website" >}}
</a>
{{< /col >}}
{{< col card="false" y="center" order="first" >}}
## Web Design & SEO for Political Campaign
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="first" >}}
## Web Design & SEO for Business Services
{{< /col >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://digitalfacilitysolutions.com/" target="_blank" rel="noopener">
	{{< img src="/media/new-local-business-utah-county.avif" alt="Digital Facility Solutions website" >}}
</a>
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://quartzworx.com/" target="_blank" rel="noopener">
	{{< img src="/media/website-design-quartz-worx.avif" alt="Quartz Worx website" >}}
</a>
{{< /col >}}
{{< col card="false" y="center" order="first" >}}
## Web Design for Home Construction
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="first" >}}
## Web Design for Fitness Center
{{< badge text="Website In Progress">}}
{{< /col >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://cedarcitystrength.pages.dev/" target="_blank" rel="noopener">
	{{< img src="/media/website-design-cedar-city-strength.avif" alt="Cedar City Strength & Conditioning website" >}}
</a>
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://blueridgeabbey.com/" target="_blank" rel="noopener">
	{{< img src="/media/website-design-blue-ridge-abbey.jpg" alt="Blue Ridge Abbey website" >}}
</a>
{{< /col >}}
{{< col card="false" y="center" order="first" >}}
## Web Design & SEO for Hospitality
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="first" >}}
## Web Design & SEO for IT Services
{{< /col >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://blueridgetech.pro/" target="_blank" rel="noopener">
	{{< img src="/media/website-design-blueridge-tech.avif" alt="BlueridgeTech website" >}}
</a>
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://novagutter.com/" target="_blank" rel="noopener">
	{{< img src="/media/nova-gutter-hero.avif" alt="Nova Gutter website" >}}
</a>
{{< /col >}}
{{< col card="false" y="center" order="first" >}}
## Web Design for Home Services
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="first" >}}
## Web Design for Hospitality
{{< /col >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://villavalenciaojai.com/" target="_blank" rel="noopener">
	{{< img src="/media/website-villa-valencia.avif" alt="Villa Valencia website" >}}
</a>
{{< /col >}}
{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="center" >}}
{{< col card="false" y="center" order="last" >}}
<a href="https://rodmaxfielddds.pages.dev/" target="_blank" rel="noopener">
	{{< img src="/media/rodmaxfieddds-hero.avif" alt="Rod Maxfield DDS website" >}}
</a>
{{< /col >}}
{{< col card="false" y="center" order="first" >}}
## Web Design for Dental Practice
{{< badge text="Website In Progress">}}

{{< /col >}}
{{< /cols >}}

## Ready to start your website?

Get a fast, effective website that helps your business grow.

{{< button url="/services/website-design" text="Website Design" >}} {{< button url="/contact" text="Contact" >}}
