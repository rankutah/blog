---
title: "Breaking Barriers Therapy  in Utah County - ABA Therapy, Occupational Therapy (OT), and Speech Therapy Services"
description: "ABA Services in Utah. From infants to adults, our therapy services encompass assessment, treatment, and skill enhancement. With care, compassion, and expertise, we make potential possible."
layout: flowbite
url: "/"
---

{{< hero img="/media/import/bg-1-1066221c.jpeg" bleed="true" overlay="true" overlayShade="bg-black/45" align="center" vh="svh" vhHeight="74" >}}

BREAKING BARRIERS THERAPY SERVICES IN UTAH COUNTY

# Where families find their village of support and peace of mind

From infants to adults, our therapy services encompass assessment, treatment, and skill enhancement. With care, compassion, and expertise, we make potential possible.

{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}
{{< button url="tel:+18019876333" text="CALL US AT 801-987-6333" >}}

{{< /hero >}}

## Diagnostic Testing for Autism and More...

{{< button url="/contact" text="SCHEDULE NOW!" >}}

{{< cols min="16rem" gap="1.5rem" v="start" stagger="false" >}}

{{< col card="true" y="start" classes="h-auto rounded-2xl border border-[#d9d2c2] bg-white/95 p-4 md:p-5 shadow-[0_18px_40px_-28px_rgba(15,59,87,0.32)] [&_img]:my-0 [&_h3]:mt-2 [&_h3]:mb-2 [&_p]:my-1 [&_p:last-child]:mb-0" >}}

{{< img src="/media/location-pin.svg" alt="Location pin" class="mx-auto w-12 h-auto rounded-none shadow-none border-0" sizes="48px" >}}

### Saratoga Springs, Utah

24 W Sergeant Ct Dr # 204, Saratoga Springs, UT 84045

[(801) 987-6333](tel:+18019876333)

{{< /col >}}

{{< col card="true" y="start" classes="h-auto rounded-2xl border border-[#d9d2c2] bg-white/95 p-4 md:p-5 shadow-[0_18px_40px_-28px_rgba(15,59,87,0.32)] [&_img]:my-0 [&_h3]:mt-2 [&_h3]:mb-2 [&_p]:my-1 [&_p:last-child]:mb-0" >}}

{{< img src="/media/location-pin.svg" alt="Location pin" class="mx-auto w-12 h-auto rounded-none shadow-none border-0" sizes="48px" >}}

### Lehi, Utah

400 Millpond Dr #E, Lehi, UT 84043

[(801) 987-6333](tel:+18019876333)

{{< /col >}}

{{< col card="true" y="start" classes="h-auto rounded-2xl border border-[#d9d2c2] bg-white/95 p-4 md:p-5 shadow-[0_18px_40px_-28px_rgba(15,59,87,0.32)] [&_img]:my-0 [&_h3]:mt-2 [&_h3]:mb-2 [&_p]:my-1 [&_p:last-child]:mb-0" >}}

{{< img src="/media/location-pin.svg" alt="Location pin" class="mx-auto w-12 h-auto rounded-none shadow-none border-0" sizes="48px" >}}

### New location coming soon!

More support for Utah families is on the way.

{{< button url="/contact" text="Ask About Availability" variant="outline" >}}

{{< /col >}}

{{< /cols >}}

{{< section y="sm" align="center" >}}

All of our therapists collaborate closely, resulting in expedited progress and results, bringing peace of mind and success to you and your family.

{{< /section >}}

{{< cols min="22rem" gap="2rem" v="center" >}}

{{< col card="false" y="center" >}}

{{< img src="/media/import/girl-playing-puzzle-for-therapy-8fec14e4.webp" alt="Child playing during therapy" class="mx-auto w-full max-w-2xl h-auto rounded-xl shadow border border-gray-200 dark:border-gray-700" sizes="(min-width: 1024px) 48vw, 100vw" >}}

{{< /col >}}

{{< col card="false" y="start" >}}

ABA services specialize in working with children from 18 months to 12 years with Autism. Our unique collaborative approach offers resources and support to parents like you. We see the best results when families are involved. We invite families to be engaged with their child's ABA therapy plan.

Rehabilitation services such as speech-language pathology, occupational therapy, and physical therapy specialize in all ages. If there are communication, feeding, development, or physical concerns, we are here to help.

{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}

{{< /col >}}

{{< /cols >}}

### Explore how we help make potential possible.
Our specialized therapy services:

{{< cols min="18rem" gap="2rem" v="start" >}}

{{< col card="true" y="start" >}}

{{< img src="/media/import/mother-helping-daughter-play-therapy--281-29-6acb3e4b.webp" alt="ABA therapy" class="w-full h-auto rounded-xl shadow border border-gray-200 dark:border-gray-700" sizes="(min-width: 1024px) 30vw, 100vw" >}}

## ABA Therapy

Are you looking to increase communication, independence, play, work, and social skills or school readiness skills?

Our naturalistic, neuro-affirming, assent-based Applied Behavior Analysis Therapy (ABA Therapy) services can help. With the right interventions in the right setting, individuals from 18-months to 18 years with ASD, ODD, ADHD, and other neurodevelopmental challenges or injuries can improve their behaviors and reach their full potential.

* Communication
* Cognitive Skills
* Social skills
* Behavior management
* Daily living skills

{{< button url="/aba-therapy" text="ABA Therapy Services" >}}

{{< /col >}}

{{< col card="true" y="start" >}}

{{< img src="/media/import/ocupational-therapy-75bb9204.webp" alt="Occupational therapy" class="w-full h-auto rounded-xl shadow border border-gray-200 dark:border-gray-700" sizes="(min-width: 1024px) 30vw, 100vw" >}}

## Occupational Therapy (OT)

Have you been looking for ways to develop, regain, or improve skills needed to perform daily tasks, such as sensory processing, emotional regulation, dressing, eating, reading, writing, and fine/gross motor function? Occupational therapy services can help. From assessment to treatment, we support infants to adults with sensory processing, sleep, selective eating, and emotion regulation concerns to enhance overall quality of life.

* Fine and gross motor skills
* Sensory processing
* Emotion regulation
* Self-care
* Balance
* Strength/Coordination
* Splinting

{{< button url="/occupational-therapy" text="Occupational Therapy Services" >}}

{{< /col >}}

{{< col card="true" y="start" >}}

{{< img src="/media/import/therapist-helping-child-speech-therapy-818a1e28.webp" alt="Speech therapy" class="w-full h-auto rounded-xl shadow border border-gray-200 dark:border-gray-700" sizes="(min-width: 1024px) 30vw, 100vw" >}}

## Speech Therapy (ST)

How can our speech-language pathologists help make a difference? From toddlers to adults, we help with challenges related to speech, language, social skills, memory/executive function, voice, tongue thrust, or swallowing disorders. Speech-language therapy helps overcome barriers and empowers individuals to communicate, process information, articulate words, communicate, and listen effectively.

* Speech and language delays/disorders
* Social pragmatic skills
* Voice pitch, volume, and overall vocal conditions
* Difficulty swallowing/tongue thrust
* Memory or problem-solving difficulties

{{< button url="/speech-language-pathology" text="Speech Therapy Services" >}}

{{< /col >}}

{{< /cols >}}

## New services coming soon!
_at all Breaking Barriers Therapy Services locations_

### Clinical Psychology | Physical Therapy | Comprehensive Assessments

<div class="mt-8 flex flex-wrap gap-2">
{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}
{{< button url="tel:+18019876333" text="CALL US AT 801-987-6333" >}}
</div>

{{< cols min="20rem" gap="2rem" v="start" >}}

{{< col card="false" y="start" >}}

## Have you been searching for support with communication, development, and behavior for your child?

Wondering if anyone has answers and solutions that can lead to consistent progress?

Families face significant changes when a child is diagnosed with autism, experiences a brain injury, or encounters developmental delays or disorders. Finding a successful path in a circle of care can be tough, isolating, and frustrating when you don't feel heard and progress is not happening.

We understand how advocating for your child requires unwavering determination and persistence - we've been there, too. There are always more answers to be found, but your fierce nature and commitment to never giving up will always drive you and your child forward.

We're here to help you move forward and make reaching potential possible. We believe it takes a village to raise a child. There is power in a united village dedicated to your child and family's best interests, all in one place.

## We are that village. Your circle of care and support.

{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}

{{< /col >}}

{{< col card="false" y="start" >}}

{{< quote author="Jen W." >}}
"...I cannot believe the progress my daughter has made! Her preschool teacher has commented, and her dad and I are blown away! Taylor is amazing with Sloane, and just what she has needed to help give her confidence and show her how to say the words and where her tongue should be instead of just telling her... She is understanding more and more and correcting herself, and it's amazing. She still has a lot to work on, but I am confident that with the help of Taylor and Breaking Barriers, she will continue to progress! Thank you!!"
{{< /quote >}}

{{< quote author="Robyn P." >}}
"My(2 1/2) son was barely saying mom, dad, dog, ball, and Z (his brother's name). He wouldn't even try to mimic what was being said to him. After just 1 session with Jenn, he said, \"I want a ball\". I was blown away! Each session after that he continued to say more and more words. But most impressive to me was his confidence... He is on track to be right where he should be with his speech for his age! I'm so incredibly happy! I've cried happy tears a number of times watching and hearing his progress! My favorite words he says now are \"thank you\" and \"I love you, Mom\". Thank you so much for giving my son the attention and love he deserved! He's a different kid!"
{{< /quote >}}

{{< /col >}}

{{< /cols >}}

## You want the best for your child, And so do we.

{{< cols min="18rem" gap="2rem" v="start" stagger="false" >}}

{{< col card="true" y="center" class="text-center" >}}

## OUR MISSION

We empower children and families to thrive, making reaching potential possible.

{{< /col >}}

{{< col card="true" y="center" class="text-center" >}}

## OUR VISION

Our vision at Breaking Barriers Therapy Services is to be a beacon of exceptional care, expanding our reach to multiple facilities and guiding children and families toward transformative growth through our outstanding collaborative services.

{{< /col >}}

{{< /cols >}}

{{< cols min="18rem" gap="2rem" v="start" stagger="false" >}}

{{< col card="true" y="start" class="text-center" >}}

## OUR VALUES

Our values are E.P.I.C.C.C

* Excellence: Highest standards of assessment, treatment, and service.
* Integrity: Transparency, honesty, and ethical decision-making and processes.
* Compassion: Empathy, understanding, and genuine care for children and parents.
* Positivity: Joyful, playful, and optimistic therapy interactions.
* Collaboration: Cooperation among therapists, caregivers, and other professionals.
* Creativity: Inspire innovative approaches, adapt to changing needs, and drive positive change.

{{< /col >}}

{{< /cols >}}

<br>
{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}

## "Coming together is a beginning, keeping together is progress and working together is success."

#### - Edward Everett Hale

## Who We Help

{{< cols min="16rem" gap="2rem" v="start" stagger="false" >}}

{{< col card="true" y="start" >}}

* Anger Management
* Anxiety
* Apraxia and Ataxia
* Articulation Disorder
* Attention Deficit Hyperactivity Disorder (ADHD)
* Autism Spectrum Disorder (ASD)
* Coexisting Medical Diagnosis and Autism (Dual or Multiple Diagnoses)
* Congenital or Hereditary Syndromes (such as Tourette, Glute 1 Deficiency)
* Developmental Delay

{{< /col >}}

{{< col card="true" y="start" >}}

* Downs Syndrome
* Dressing/Clothing Issues
* Dysgraphia
* Emotional Regulation
* Environmental Modification Assessment/Support
* Executive Function Deficient (Planning, Organizing, Initiation, Memory, etc)
* Feeding Difficulties
* Gross and Fine Motor Weaknesses
* Handwriting
* Oppositional Defiance Disorder (ODD)

{{< /col >}}

{{< col card="true" y="start" >}}

* Potty Training Difficulty
* Personal Hygiene
* Receptive Language Delays/Disorders
* Sensory Processing Disorder (SPD)
* Specific Learning Disorders, such as Dyslexia
* Stroke
* Tongue Thrust
* Traumatic Brain Injury (TBI)
* Visual Processing Concerns

{{< /col >}}

{{< /cols >}}

<div class="mt-8">
{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}
</div>

## What We Provide

Wonderful life-changing moments are possible. Our key to ongoing success allows children to make progress faster.

{{< cols min="20rem" gap="2rem" v="center" >}}

{{< col card="true" y="start" >}}

* We help parents feel more confident in managing life with their child.
* We provide pediatric therapy aimed at increasing hope and success.
* We support the family and help build safe relationships in and outside the home.
* Create a stronger connection with your child.
* We teach understanding, skills, and techniques to achieve progress at home and in the world.

{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}

{{< /col >}}

{{< col card="false" y="center" >}}

{{< img src="/media/import/Untitled--281000-x-1000-px-29--284-29-8384d229.png" alt="What we provide" class="mx-auto w-full max-w-sm h-auto rounded-none shadow-none border-0" sizes="220px" >}}

{{< /col >}}

{{< /cols >}}

{{< section img="/media/import/bg-17-cd5ed693.png" bleed="false" overlay="true" overlayShade="bg-black/40" overlayClasses="absolute inset-0 flex items-center px-8 md:px-14 lg:px-20" align="center" >}}

## You are not alone.
You've come to the right place.
We can help make potential possible.

{{< /section >}}

## Ready to join our village?
3 simple steps to get started:

{{< cols min="16rem" gap="2rem" v="start" stagger="false" >}}

{{< col card="true" y="start" >}}

### STEP 1
Schedule a Free Consultation

Contact us by [phone](tel:+18019876333) or email to discuss how we can help decide the next steps forward.

{{< /col >}}

{{< col card="true" y="start" >}}

### STEP 2
Connect & Plan

Meet with our expert therapist for an assessment so we can customize an individual plan for your child.

{{< /col >}}

{{< col card="true" y="start" >}}

### STEP 3
Explore Possibilities

Follow the guidance and plan we create together. Start seeing positive changes and celebrate every success!

{{< /col >}}

{{< /cols >}}

<br>
{{< contact-form id="home-consultation" phone="optional" preferredContact="optional" message="optional" >}}

{{< button submit="true" form="home-consultation" text="Request Consultation" >}}

<div class="mt-8 flex flex-wrap gap-2">
{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}
{{< button url="tel:+18019876333" text="CALL US AT 801-987-6333" >}}
</div>

{{< cols min="7rem" gap="0.75rem" v="center" stagger="false" >}}

{{< col card="false" y="center" >}}
{{< img src="/media/import/ASHA_Certified_Logo_Black-2-2930679a.png" alt="ASHA certified logo" class="mx-auto w-auto max-w-[5.5rem] sm:max-w-[6.5rem] md:max-w-[8rem] h-auto rounded-none shadow-none border-0" sizes="(min-width: 768px) 128px, 88px" >}}
{{< /col >}}

{{< col card="false" y="center" >}}
{{< img src="/media/import/BACB_Logo_200-b12a2bd7.png" alt="BACB logo" class="mx-auto w-auto max-w-[5.5rem] sm:max-w-[6.5rem] md:max-w-[8rem] h-auto rounded-none shadow-none border-0" sizes="(min-width: 768px) 128px, 88px" >}}
{{< /col >}}

{{< col card="false" y="center" >}}
{{< img src="/media/import/Member_Logo_CASP-5f306bf6.png" alt="CASP member logo" class="mx-auto w-auto max-w-[5.5rem] sm:max-w-[6.5rem] md:max-w-[8rem] h-auto rounded-none shadow-none border-0" sizes="(min-width: 768px) 128px, 88px" >}}
{{< /col >}}

{{< col card="false" y="center" >}}
{{< img src="/media/import/usha_LOGO_2019apr24-c921aae4.png" alt="USHA logo" class="mx-auto w-auto max-w-[5.5rem] sm:max-w-[6.5rem] md:max-w-[8rem] h-auto rounded-none shadow-none border-0" sizes="(min-width: 768px) 128px, 88px" >}}
{{< /col >}}

{{< col card="false" y="center" >}}
{{< /col >}}

{{< /cols >}}

{{< section y="sm" align="center" >}}

## Every child deserves to learn the unique skills to perform successfully at home and in the world.

## Every parent deserves to find peace and confidence to be the guide their child needs.

_Let's discuss how we can help._

{{< /section >}}

<div class="mt-8">
{{< button url="/contact" text="SCHEDULE FREE CONSULTATION" >}}
</div>
